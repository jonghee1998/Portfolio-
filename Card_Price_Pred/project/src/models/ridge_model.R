...
# Find minimum lambda using cv , and create best fit gl_model (Ridge model: alpha level 0)
...
# Create submit data table of ridge model
submit <- test[,.(id,price)]

# Save files
saveRDS(gl_model, "./project/volume/models/gl_model_ridge.model")
fwrite(submit,"./project/volume/data/processed/submit_ridge.csv")
